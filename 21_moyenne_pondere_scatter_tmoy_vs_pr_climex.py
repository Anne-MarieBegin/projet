...
######################################################################
if b_pt == 'brute':
    df_diff=pd.read_csv('/tank/begin/weighting/resultats/pond_exp_pente_obs_b_tmoy_'+saison[s]+'_'+rcp+'.csv')
else:
    df_diff=pd.read_csv('/tank/begin/weighting/resultats/pond_exp_pente_obs_pt_tmoy_'+saison[s]+'_'+rcp+'.csv')
...
for i in range(len(df1)):
    if df1.groupe[i]=='ClimEx':
        climex.append(df1['membre'][i])
for i in climex:    
    if b_pt=='brute':
        df_diff_k=pd.read_csv('/tank/begin/weighting/resultats/pond_exp_pente_'+i+'_b_tmoy_'+saison[s]+'_'+rcp+'.csv')
    else:
        df_diff_k=pd.read_csv('/tank/begin/weighting/resultats/pond_exp_pente_'+i+'_pt_tmoy_'+saison[s]+'_'+rcp+'.csv')
 
    w_diff_k=df_diff_k['0']
    
    dff1_=df1.iloc[position].reset_index()
    moyd=[]
    for j in range(len(dff1_)):
        moyd.append(dff1_['data'][j][s]*w_diff_k[j])
    moy_pond_diff_k_1=(np.sum(moyd)/np.sum(w_diff_k))
    
    dff2_=df2.iloc[position].reset_index()
    moyd=[]
    for j in range(len(dff2_)):
        moyd.append(dff2_['data'][j][s]*w_diff_k[j])
    moy_pond_diff_k_2=(np.sum(moyd)/np.sum(w_diff_k))
    
    dff3_=df3.iloc[position].reset_index()
    moyd=[]
    for j in range(len(dff3_)):
        moyd.append(dff3_['data'][j][s]*w_diff_k[j])
    moy_pond_diff_k_3=(np.sum(moyd)/np.sum(w_diff_k))
    
    delta_moy_pond_diff_k_1=moy_pond_diff_k_2-moy_pond_diff_k_1
    delta_moy_pond_diff_k_2=moy_pond_diff_k_3-moy_pond_diff_k_1
    
    dff1p_=df1p.iloc[position].reset_index()
    moyd=[]
    for j in range(len(dff1p_)):
        moyd.append(dff1p_['data'][j][s]*w_diff_k[j])
    moy_pond_diff_k_1p=(np.sum(moyd)/np.sum(w_diff_k))
    
    dff2p_=df2p.iloc[position].reset_index()
    moyd=[]
    for j in range(len(dff2p_)):
        moyd.append(dff2p_['data'][j][s]*w_diff_k[j])
    moy_pond_diff_k_2p=(np.sum(moyd)/np.sum(w_diff_k))
    
    dff3p_=df3p.iloc[position].reset_index()
    moyd=[]
    for j in range(len(dff3p_)):
        moyd.append(dff3p_['data'][j][s]*w_diff_k[j])
    moy_pond_diff_k_3p=(np.sum(moyd)/np.sum(w_diff_k))
    
    delta_moy_pond_diff_k_1p=(moy_pond_diff_k_2p-moy_pond_diff_k_1p)/moy_pond_diff_k_1p*100
    delta_moy_pond_diff_k_2p=(moy_pond_diff_k_3p-moy_pond_diff_k_1p)/moy_pond_diff_k_1p*100
    
    plt.figure(1) 
    
    plt.scatter(delta_moy_pond_diff_k_1,delta_moy_pond_diff_k_1p,color='grey')
    ...
    plt.scatter(delta_moy_pond_diff_1,delta_moy_pond_diff_1p,color='lightgrey')
    
    legend=['moyenne pondérée_exp_pente_tmoy_climex_'+saison[s],'moyenne non pondérée',
            'moyenne pondérée_250000km²(5$\Delta$x)',
            'moyenne pondérée_1m1v_uniforme','moyenne pndérée_1c1v',
            'CMIP5','CORDEX','ClimEx','moyenne pondérée_exp_pente_tmoy_obs_'+saison[s]]
    ...
    if b_pt=='brute':
        ...
        plt.savefig('/tank/begin/weighting/plots/brute/pr_tmoy_b_'+SE+'_delta_exp_pente_climex_tmoy_2041_2070_'+saison[s]+'_'+rcp,bbox_inches='tight')
    else:
        plt.title('Delta entre 1981-2010 et 2041_2070\n'+saison[s]+'_'+rcp+' ('+SE+')\nPost-traitées')
        plt.savefig('/tank/begin/weighting/plots/posttraite/pr_tmoy_pt_'+SE+'_delta_exp_pente_climex_tmoy_2041_2070_'+saison[s]+'_'+rcp,bbox_inches='tight')
    #
    plt.figure(2) 
    
    plt.scatter(delta_moy_pond_diff_k_2,delta_moy_pond_diff_k_2p,color='grey')
    ...
    plt.scatter(delta_moy_pond_diff_2,delta_moy_pond_diff_2p,color='lightgrey')
    ...
    if b_pt=='brute':
        ...
        plt.savefig('/tank/begin/weighting/plots/brute/pr_tmoy_b_'+SE+'_delta_exp_pente_climex_tmoy_2071_2100_'+saison[s]+'_'+rcp,bbox_inches='tight')
    else:
        plt.title('Delta entre 1981-2010 et 2071-2100\n'+saison[s]+'_'+rcp+' ('+SE+')\nPost-traitées')
        plt.savefig('/tank/begin/weighting/plots/posttraite/pr_tmoy_pt_'+SE+'_delta_exp_pente_climex_tmoy_2071_2100_'+saison[s]+'_'+rcp,bbox_inches='tight')
