...
fit_da_obs=[]
for j in range(len(da_obs)):
    df_obs_sans_nan = [x for x in df_obs[j][s] if np.isnan(x) == False]
    fit_da_obs.append(np.polyfit(x,df_obs_sans_nan,1)[0])#[0]pour obtenir la pente)


plt.plot(np.arange(1971,2001),df_obs[0][s],label='NRcan/info-crue pente='+str(round(fit_da_obs[0],4)))
plt.plot(np.arange(1971,2001),df_obs[1][s],label='NRcan pente='+str(round(fit_da_obs[1],4)))
plt.legend()
plt.ylabel('Temp moyenne $\circ$C')
plt.title('Comparaison NRcan\n(E_1) 1971-2000_'+saison[s],fontsize=15)
plt.savefig('/tank/begin/weighting/plots/'+var+'_E_1_comparaison_nrcan_1971-2000_'+saison[s],bbox_inches='tight')
