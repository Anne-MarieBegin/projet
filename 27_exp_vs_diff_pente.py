...
for i in range(len(df_all)):
    if df_all.groupe[i]=='ClimEx':
        sans_nan=[x for x in df_all.data[i][s] if np.isnan(x) == False]
        fit_da_climex.append(np.polyfit(x,sans_nan,1)[0])
...
#calcul de exponentielle (poids) simulation
w_all=[]
...
if b_pt == 'brute':
    ...
    plt.savefig('/tank/begin/weighting/plots/brute/'+var+'_E_1_b_pond_expvsdiff_pente_obs_1m1v_1971-2017_'+saison[s]+'_'+rcp+'_sa_al',bbox_inches='tight')
else:
    plt.title('Poids référence NRcan\n'+var+' pente tendance (E_1) 1971-2017_'+saison[s]+'_'+rcp+'\nPost-traitées\nsigma_d='+str(round(sigma_d,3)),fontsize=15)
    plt.savefig('/tank/begin/weighting/plots/posttraite/'+var+'_E_1_pt_pond_expvsdiff_pente_obs_1m1v_1971-2017_'+saison[s]+'_'+rcp+'_sa_al',bbox_inches='tight')
