...
#calcul pente tendance climex
fit_climex=[]
climex_=[]
for i in range(len(df_all)):
    if df_all.groupe[i]=='ClimEx':
        sans_nan=[x for x in df_all.data[i][s] if np.isnan(x) == False]
        fit_climex.append(np.polyfit(x,sans_nan,1)[0])#[0]pour obtenir la pente
        climex_.append(df_all.iden[i][-16:-13])
        
diff_climex=[]
for i in range(len(fit_climex)):
    diff_climex.append(abs(fit_climex[i]-fit_da_obs))
...
for c in range(0,50):    
    ...
    #calcul pente tendance tous l'ensemble
    fit_da_all=[]
    identifiant=[]
    for i in range(len(df_all)):
        if df_all.rcp[i]==rcp:
    ...
    for j in range(len(fit_da_all)):
        diff_fit_all.append(abs(fit_da_all[j]-fit_climex[c]))
    ...
    if b_pt=='brute':
        df_w_all_norm.to_csv('/tank/begin/weighting/resultats/pond_exp_pente_'+climex_[c]+'_b_'+var+'_'+saison[s]+'_'+rcp+'.csv')
    else:
        df_w_all_norm.to_csv('/tank/begin/weighting/resultats/pond_exp_pente_'+climex_[c]+'_pt_'+var+'_'+saison[s]+'_'+rcp+'.csv')
    ...
    if b_pt == 'brute':
        plt.title('Poids référence ClimEx\n'+var+' pente tendance (E_1) 1971-2000 '+saison[s]+'_'+rcp+'\nBrutes\nsigma_d='+str(round(sigma_d,3)),fontsize=15)
        plt.savefig('/tank/begin/weighting/plots/brute/'+var+'_E_1_b_pond_exp_pente_climex_1971-2000_'+saison[s]+'_'+rcp,bbox_inches='tight')
    else:
        plt.title('Poids référence ClimEx\n'+var+' pente tendance (E_1) 1971-2000 '+saison[s]+'_'+rcp+'\nPost-traitées\nsigma_d='+str(round(sigma_d,3)),fontsize=15)
        plt.savefig('/tank/begin/weighting/plots/posttraite/'+var+'_E_1_pt_pond_exp_pente_climex_1971-2000_'+saison[s]+'_'+rcp,bbox_inches='tight')
