...
df=pd.DataFrame(moy,ecs).reset_index()
#ajouter colonne gcm
df['g']=gcm
...
if b_pt=='brute':
    ...
    plt.savefig('/tank/begin/weighting/plots/brute/'+var+'_b_ECS_VS_'+stat3[-10:-1]+'_'+rcp+'_'+saison[s],bbox_inches='tight')
else:
    plt.title(var+'_'+saison[s]+'\n'+stat3[-10:-1]+'_'+rcp+'\nCoefficient corrélation ='+str(round(cc[0][1],3))+'\nPost-traitées')  
    plt.savefig('/tank/begin/weighting/plots/posttraite/'+var+'_pt_ECS_VS_'+stat3[-10:-1]+'_'+rcp+'_'+saison[s],bbox_inches='tight')
