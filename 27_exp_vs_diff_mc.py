...
for j in range(len(moy_da_all)):
    diff_moy_all.append(abs(moy_da_all[j]-moy_da_obs))


diff_climex=[]
for i in range(len(df_all)):
    if df_all.groupe[i]=='ClimEx':
        diff_climex.append(abs((np.nanmean(df_all.data[i][s]))-moy_da_obs))

sigma_d=np.std(diff_climex)


climex=[]
for i in range(len(df_all)):
    if df_all.groupe[i]=='ClimEx':
        climex.append(np.nanmean(df_all.data[i][s]))
std_climex=np.std(climex)
sigma_climex=np.sqrt(2)*std_climex

sigma_d=0.5
#calcul de 1/différence (poids) simulation
w_all=[]
for k in range(len(diff_moy_all)):
    w_all.append(np.exp(-(diff_moy_all[k]/sigma_d)**2))
...
w_all_norm_diff=[]
for k in range(len(diff_moy_all)):
    w_all_norm_diff.append(w_all[k]/sum_diff)
##dataframe avec poids normalisés
df_w_all_norm_diff=pd.DataFrame(w_all_norm_diff)
df_w_all_norm['iden']=identifiant 
##enregistrer poids normalisés
#if b_pt=='brute':
#    df_w_all_norm.to_csv('/tank/begin/weighting/resultats/pond_exp_mc_obs_b_'+var+'_'+saison[s]+'_'+rcp+'.csv')
#else:
#    df_w_all_norm.to_csv('/tank/begin/weighting/resultats/pond_exp_mc_obs_pt_'+var+'_'+saison[s]+'_'+rcp+'.csv')
...
combine_diff=[[]for k in range(len(df_unique))]
for k in range(len(df_w_all_norm)):  
    for i in range(len(df_unique)):
        if df_unique[i]==identifiant[k]:
            combine_diff[i].append(df_w_all_norm_diff[0][k])
            combine_iden[i].append(df_w_all_norm['iden'][k])
...
for i in range(len(df_unique)):
    for j in range(len(combine[i])):
        ...
        if 'Ouranos_'in df_unique[i]:
            plt.plot(df_unique[i],combine[i][j],'.',color='r')
for i in range(len(df_unique)):
    for j in range(len(combine_diff[i])):
        plt.plot(df_unique[i],combine_diff[i][j],'.',color='g')
        if df_unique[i]=='ClimEx_CanESM2_CRCM5-Ouranos':
            plt.plot(df_unique[i],combine_diff[i][j],'.',color='orange')
        if 'CORDEX'in df_unique[i]:
            plt.plot(df_unique[i],combine_diff[i][j],'.',color='c')
        if 'Ouranos_'in df_unique[i]:
            plt.plot(df_unique[i],combine_diff[i][j],'.',color='c')
if b_pt == 'brute':
    plt.title('Poids référence NRcan\n'+var+' moyenne climatique (E_1) 1971-2000_'+saison[s]+'_'+rcp+'\nBrutes\n sigma_d='+str(round(sigma_d,2)),fontsize=15)
    plt.savefig('/tank/begin/weighting/plots/brute/'+var+'_E_1_b_pond_expvsdiff_mc_obs_1m1v_1971-2000_'+saison[s]+'_'+rcp+'_sa_al',bbox_inches='tight')
else:
    plt.title('Poids référence NRcan\n'+var+' moyenne climatique (E_1) 1971-2000_'+saison[s]+'_'+rcp+'\nPost-traitées\nsigma_d='+str(round(sigma_d,2)),fontsize=15)
    plt.savefig('/tank/begin/weighting/plots/posttraite/'+var+'_E_1_pt_pond_expvsdiff_mc_obs_1m1v_1971-2000_'+saison[s]+'_'+rcp+'_sa_al',bbox_inches='tight')
