...
v20210106 (brute et post traite) changer titre erreur sur la moyenne normalisée
"""
...
for j in range(0,(len(files2))):
    da2.append(xr.open_dataarray(path2+'/'+files2[j])-273.15)
for j in range(0,(len(files_obs2))):
    da_obs2.append(xr.open_dataarray(path_obs2+'/'+files_obs2[j]))

#insert obs at the first position
da2.insert(0,da_obs2[0])   
     
######################################################################
#calcul écart type cycle interannuel 
std=[]
for i in range(0,len(files2)+1):
    std.append(np.std(da2[i]))

#calcul moyenne cycle annuel
moy=[]
for i in range(0,len(files)+1):
    moy.append(np.mean(da[i]))
    
#erreur sur la moyenne normaliser par écart type cycle interannuel     
erreur_moy=[]
for i in range(0,len(files)+1):
    erreur_moy.append(((moy[i]-moy[0])/(std[0])))
...
#calcul erreur relative sur la variance
erreur_var=[]
for j in range(0,len(files)+1):
...
for p in range(0,len(files2)+1):
    for t in range(0,28):
        yy[p].append((pente[p]/10)*x[t]+oo[p])#(/10 pour compenser pour ligne 109)
...
#calcul erreur relative sur le pente tendance 10 ans
erreur_pente=[]
...
if b_pt == 'brute':
    plt.title(var+' \n1971-2000*\nBrutes')
    plt.savefig('/exec/begin/weighting/plots/brute/'+var+'_SE_1_b_erreur_moy_var_tendance_1971-2000',bbox_inches='tight')
else:
    plt.title(var+' \n1971-2000*\nPost-traitées')
    plt.savefig('/exec/begin/weighting/plots/posttraite/'+var+'_SE_1_pt_erreur_moy_var_tendance_1971-2000',bbox_inches='tight')
