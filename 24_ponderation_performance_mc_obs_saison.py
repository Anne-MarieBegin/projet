...
#calcul moyenne climatique observation
moy_da_obs=np.nanmean(df_obs[s])


#calcul moyenne climatique ensemble
moy_da_all=[]
...
if b_pt == 'brute':
    df_w_all_norm.to_csv('/tank/begin/weighting/resultats/pond_performance_mc_obs_b_'+var+'_'+saison[s]+'.csv')
else:
    df_w_all_norm.to_csv('/tank/begin/weighting/resultats/pond_performance_mc_obs_pt_'+var+'_'+saison[s]+'.csv')
...
plt.grid()
if b_pt == 'brute':
    plt.title('Poids - référence RNcan\n'+var+' moyenne climatique (E_1) 1971-2000_'+saison[s]+'\nBrutes',fontsize=30)
    plt.savefig('/tank/begin/weighting/plots/brute/'+var+'_E_1_b_pond_performance_mc_obs_1971-2000_'+saison[s],bbox_inches='tight')
else:
    plt.title('Poids - référence RNcan\n'+var+' moyenne climatique (E_1) 1971-2000_'+saison[s]+'\nPost-traitées',fontsize=30)
    plt.savefig('/tank/begin/weighting/plots/posttraite/'+var+'_E_1_pt_pond_performance_mc_obs_1971-2000_'+saison[s],bbox_inches='tight')
