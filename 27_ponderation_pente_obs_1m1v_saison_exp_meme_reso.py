...
if s ==0:
    x=np.arange(1971,1998)
else:
    x=np.arange(1971,1999)
    
fit_da_obs=[]
for j in range(len(df_obs)):
    df_obs_sans_nan = [x for x in df_obs.data[j][s] if np.isnan(x) == False]
...
for j in range(len(fit_da_climex)):
    diff_climex.append(abs(fit_da_climex[j]-fit_da_obs[41]))#10 correspond a climex dans observation tmoy
                                                            #41 correspond a climex dans observation pr
...
if b_pt=='brute':
    df_w_all_norm.to_csv('/tank/begin/weighting/resultats/pond_exp_pente_obs_b_'+var+'_'+saison[s]+'_'+rcp+'.csv')
else:
    df_w_all_norm.to_csv('/tank/begin/weighting/resultats/pond_exp_pente_obs_pt_'+var+'_'+saison[s]+'_'+rcp+'.csv')
...
if b_pt == 'brute':
    ...
    plt.savefig('/tank/begin/weighting/plots/brute/'+var+'_E_1_b_pond_exp_pente_obs_1971-2000_'+saison[s]+'_'+rcp,bbox_inches='tight')
else:
    plt.title('Poids référence NRcan\n'+var+' pente tendance (E_1) 1971-2000_'+saison[s]+'_'+rcp+'\nPost-traitées\nsigma_d='+str(round(sigma_d,3)),fontsize=15)
    plt.savefig('/tank/begin/weighting/plots/posttraite/'+var+'_E_1_pt_pond_exp_pente_obs_1971-2000_'+saison[s]+'_'+rcp,bbox_inches='tight')
