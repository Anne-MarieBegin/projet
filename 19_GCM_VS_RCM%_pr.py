...
for i in range(len(df2)):
    for j in range(len(GCM)):
        if df2['gcm'][i] == GCM[j] and df2['rcp'][i]==rcp and df2['rcm'][i]=='NotApplicable':
            gcm_data[j].append((df2['data'][i][s]-df1['data'][i][s])/df1['data'][i][s]*100)
           # print(i,df1['rcm'][i])
...
for i in range(len(df2)):
    for j in range(len(df)):
        if df2['gcm'][i] == df['index'][j] and df2['rcm'][i]!='NotApplicable'and df2['rcp'][i]==rcp:
            rcm_data[j].append((df2['data'][i][s]-df1['data'][i][s])/df1['data'][i][s]*100)
            df_data[j].append(df[0][j])
            
            #plot gcm vs rcm
            if df2['groupe'][i]=='ClimEx':
                color='grey'
            elif df2['rcm'][i]==('CRCM5-Ouranos'):
                color='g'
            elif df2['rcm'][i]==('CRCM5-UQAM'):
                color='g'
            elif df2['rcm'][i]=='CanRCM4':
                color='r'
            elif df2['rcm'][i]=='HIRHAM5':
                color='c'
            elif df2['rcm'][i]=='RCA4':
                color='b'
            elif df2['rcm'][i]=='RegCM4':
                color='m'
            else:
                color='y'
            
            plt.scatter(df[0][j],(df2['data'][i][s]-df1['data'][i][s])/df1['data'][i][s]*100,c=color,marker='x')

colors = ['g', 'r', 'b']#,'m','grey']
lines = [Line2D([0], [0], color=c, marker='x',lw=0) for c in colors]
labels = ['CRCM5', 'CanRCM4', 'RCA4']#,'RegCM4','ClimEx']
...
plt.plot(np.arange(0,20,1),np.arange(0,20,1),color='k',linewidth=0.5)            
plt.ylabel('Delta précipitation du RCM (%)')
plt.xlabel('Delta précipitation du GCM (%)')
           
if b_pt=='brute':
    plt.title(var+'_'+stat2[-10:-1]+'_'+saison[s]+'_'+rcp+'\ncoefficient corélation='+str(round(cc[0][1],3))+'\nBrutes')
    plt.savefig('/tank/begin/weighting/plots/brute/'+var+'_b_GCM_vs_RCM_er_'+stat2[-10:-1]+'_'+rcp+'_'+saison[s],bbox_inches='tight')
else:
    plt.title(var+'_'+stat2[-10:-1]+'_'+saison[s]+'_'+rcp+'\ncoefficient corélation='+str(round(cc[0][1],3))+'\nPost-traitées')
    plt.savefig('/tank/begin/weighting/plots/posttraite/'+var+'_pt_GCM_vs_RCM_er_'+stat2[-10:-1]+'_'+rcp+'_'+saison[s],bbox_inches='tight')
##################################################################################
#liste GCM et RCM
...
for i in range(len(df3)):
    for j in range(len(GCM)):
        if df3['gcm'][i] == GCM[j] and df3['rcp'][i]==rcp and df3['rcm'][i]=='NotApplicable':
            gcm_data[j].append((df3['data'][i][s]-df1['data'][i][s])/df1['data'][i][s]*100)
           # print(i,df1['rcm'][i])
...
for i in range(len(df3)):
    for j in range(len(df)):
        if df3['gcm'][i] == df['index'][j] and df3['rcm'][i]!='NotApplicable'and df3['rcp'][i]==rcp:
            rcm_data[j].append((df3['data'][i][s]-df1['data'][i][s])/df1['data'][i][s]*100)
            df_data[j].append(df[0][j])
            
            #plot gcm vs rcm
            if df3['groupe'][i]=='ClimEx':
                color='grey'
            elif df3['rcm'][i]=='CRCM5-Ouranos':
                color='g'
            elif df3['rcm'][i]=='CRCM5-UQAM':
                color='g'
            elif df3['rcm'][i]=='CanRCM4':
                color='r'
            elif df3['rcm'][i]=='HIRHAM5':
                color='c'
            elif df3['rcm'][i]=='RCA4':
                color='b'
            elif df3['rcm'][i]=='RegCM4':
                color='m'
            else:
                color='y'
            
            plt.scatter(df[0][j],(df3['data'][i][s]-df1['data'][i][s])/df1['data'][i][s]*100,c=color,marker='x')
...
plt.plot(np.arange(-5,20,1),np.arange(-5,20,1),color='k',linewidth=0.5)            
plt.ylabel('Delta précipitation du RCM (%)')
plt.xlabel('Delta précipitation du GCM (%)')
           
if b_pt=='brute':
    plt.title(var+'_'+stat3[-10:-1]+'_'+saison[s]+'_'+rcp+'\ncoefficient corélation='+str(round(cc[0][1],3))+'\nBrutes')
    plt.savefig('/tank/begin/weighting/plots/brute/'+var+'_b_GCM_vs_RCM_er_'+stat3[-10:-1]+'_'+rcp+'_'+saison[s],bbox_inches='tight')
else:
    plt.title(var+'_'+stat3[-10:-1]+'_'+saison[s]+'_'+rcp+'\ncoefficient corélation='+str(round(cc[0][1],3))+'\nPost-traitées')
    plt.savefig('/tank/begin/weighting/plots/posttraite/'+var+'_pt_GCM_vs_RCM_er_'+stat3[-10:-1]+'_'+rcp+'_'+saison[s],bbox_inches='tight')
