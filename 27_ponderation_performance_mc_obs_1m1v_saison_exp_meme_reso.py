...
moy_da_obs=np.nanmean(df_obs.data[41][s])#10 obs tmoy climex nam-11
                                         #41 obs pr climex nam-11
    ...
    for j in range(len(df_obs)):
        if df_all.rcp[i]==rcp:
            if df_all['reso'][i]==df_obs['reso'][j]:
                print(i,j)
                diff_moy_all.append((abs(np.nanmean(df_all['data'][i][s])-np.nanmean(df_obs['data'][j][s]))))
                identifiant.append(df_all['combine'][i])
...
sigma_d=(np.std(diff_climex))*np.sqrt(2)*2
...
if b_pt=='brute':
    df_w_all_norm.to_csv('/tank/begin/weighting/resultats/pond_exp_sigmadx2_mc_obs_b_'+var+'_'+saison[s]+'_'+rcp+'.csv')
else:
    df_w_all_norm.to_csv('/tank/begin/weighting/resultats/pond_exp_sigmadx2_mc_obs_pt_'+var+'_'+saison[s]+'_'+rcp+'.csv')
...
if b_pt == 'brute':
    ...
    #plt.savefig('/tank/begin/weighting/plots/brute/'+var+'_E_1_b_pond_exp_mc_obs_1971-2000_'+saison[s]+'_'+rcp,bbox_inches='tight')
else:
    plt.title('Poids référence NRcan\n'+var+' moyenne climatique (E_1) 1971-2000_'+saison[s]+'_'+rcp+'\nPost-traitées\nsigma_d='+str(round(sigma_d,2)),fontsize=15)
    #plt.savefig('/tank/begin/weighting/plots/posttraite/'+var+'_E_1_pt_pond_exp_mc_obs_1971-2000_'+saison[s]+'_'+rcp,bbox_inches='tight')
