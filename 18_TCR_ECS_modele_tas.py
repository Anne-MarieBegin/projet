...
for i in range(len(df1)):
    for j in range(len(gcm)):
        if (df1['gcm'][i]==gcm[j])&(df1['resolution'][i]=='NotApplicable')&(df1['rcp'][i]==rcp):
            print(i,gcm[j])
            ii[j].append((df1['data'][i][s]))

#faire moyenne chaque niveau dans liste ii
moy=[]
for i in ii:
    moy.append(np.mean(i))

#faire dataframe qui combine moy et tcr/ecs
df=pd.DataFrame(moy,tcr).reset_index()
#ajouter colonne gcm
df['g']=gcm
...
if b_pt=='brute':
   ...
   # plt.savefig('/tank/begin/weighting/plots/brute/'+var+'_b_TCR_VS_'+stat3[-10:-1]+'_'+rcp+'_'+saison[s],bbox_inches='tight')
else:
    plt.title(var+'_'+saison[s]+'\n'+stat3[-10:-1]+'_'+rcp+'\nCoefficient corrélation ='+str(round(cc[0][1],3))+'\nPost-traitées')  
   # plt.savefig('/tank/begin/weighting/plots/posttraite/'+var+'_pt_TCR_VS_'+stat3[-10:-1]+'_'+rcp+'_'+saison[s],bbox_inches='tight')
