...
######################################################################
if b_pt == 'brute':
    df_diff=pd.read_csv('/tank/begin/weighting/resultats/pond_performance_mc_obs_b_tmoy_'+saison[s]+'_'+rcp+'.csv')
else:
    df_diff=pd.read_csv('/tank/begin/weighting/resultats/pond_performance_mc_obs_pt_tmoy_'+saison[s]+'_'+rcp+'.csv')
...
######################################################################
if b_pt == 'brute':
    df_exp_mc=pd.read_csv('/tank/begin/weighting/pond_combine/pond_nn_exp_mc_71_00_obs_b_tmoy_'+saison[s]+'_'+rcp+'.csv')
    df_exp_pente=pd.read_csv('/tank/begin/weighting/pond_combine/pond_nn_exp_pente_71_17_obs_b_tmoy_'+saison[s]+'_'+rcp+'.csv')

else:
    df_exp_mc=pd.read_csv('/tank/begin/weighting/pond_combine/pond_nn_exp_mc_71_00_obs_pt_tmoy_'+saison[s]+'_'+rcp+'.csv')
    df_exp_pente=pd.read_csv('/tank/begin/weighting/pond_combine/pond_nn_exp_pente_71_17_obs_pt_tmoy_'+saison[s]+'_'+rcp+'.csv')

w_exp_mc=df_exp_mc['0']
w_exp_pente=df_exp_pente['0']
w_exp=[]
for i in range(len(w_exp_mc)):
    w_exp.append(w_exp_mc[i]*w_exp_pente[i])

   
dff1_=df1.iloc[position].reset_index()
moyd=[]
for j in range(len(dff1_)):
    moyd.append(dff1_['data'][j][s]*w_exp[j])
moy_pond_exp_1=(np.sum(moyd)/np.sum(w_exp))

dff2_=df2.iloc[position].reset_index()
moyd=[]
for j in range(len(dff2_)):
    moyd.append(dff2_['data'][j][s]*w_exp[j])
moy_pond_exp_2=(np.sum(moyd)/np.sum(w_exp))

dff3_=df3.iloc[position].reset_index()
moyd=[]
for j in range(len(dff3_)):
    moyd.append(dff3_['data'][j][s]*w_exp[j])
moy_pond_exp_3=(np.sum(moyd)/np.sum(w_exp))

delta_moy_pond_exp_1=moy_pond_exp_2-moy_pond_exp_1
delta_moy_pond_exp_2=moy_pond_exp_3-moy_pond_exp_1

dff1p_=df1p.iloc[position].reset_index()
moyd=[]
for j in range(len(dff1p_)):
    moyd.append(dff1p_['data'][j][s]*w_exp[j])
moy_pond_exp_1p=(np.sum(moyd)/np.sum(w_exp))

dff2p_=df2p.iloc[position].reset_index()
moyd=[]
for j in range(len(dff2p_)):
    moyd.append(dff2p_['data'][j][s]*w_exp[j])
moy_pond_exp_2p=(np.sum(moyd)/np.sum(w_exp))

dff3p_=df3p.iloc[position].reset_index()
moyd=[]
for j in range(len(dff3p_)):
    moyd.append(dff3p_['data'][j][s]*w_exp[j])
moy_pond_exp_3p=(np.sum(moyd)/np.sum(w_exp))

delta_moy_pond_exp_1p=(moy_pond_exp_2p-moy_pond_exp_1p)/moy_pond_exp_1p*100
delta_moy_pond_exp_2p=(moy_pond_exp_3p-moy_pond_exp_1p)/moy_pond_exp_1p*100
...
legend=['moyenne non pondérée','moy pondérée_250000km²(5$\Delta$x)',
        ...
        'CMIP5','CORDEX','ClimEX','moy_pondérée_moyenne_climatique_tmoy_obs_'+saison[s],'moy_pond_exp_mixte_tmoy_obs_'+saison[s]
        ]
...
plt.scatter(delta_moy_pond_diff_1,delta_moy_pond_diff_1p,color='orange')
plt.scatter(delta_moy_pond_exp_1,delta_moy_pond_exp_1p,color='hotpink')
...
if b_pt=='brute':
   ...
   # plt.savefig('/tank/begin/weighting/plots/brute/pr_tmoy_b_'+SE+'_moy_pondere_deltas_exp_mc_tmoy_2041_2070_'+saison[s]+'_'+rcp,bbox_inches='tight')
else:
    plt.title('Delta entre 1981-2010 et 2041_2070\n'+saison[s]+'_'+rcp+' ('+SE+')\nPost-traitées')
   # plt.savefig('/tank/begin/weighting/plots/posttraite/pr_tmoy_pt_'+SE+'_moy_pondere_deltas_exp_mc_tmoy_2041_2070_'+saison[s]+'_'+rcp,bbox_inches='tight')
...
plt.scatter(delta_moy_pond_diff_2,delta_moy_pond_diff_2p,color='orange')
plt.scatter(delta_moy_pond_exp_2,delta_moy_pond_exp_2p,color='hotpink')
...
if b_pt=='brute':
   ...
   # plt.savefig('/tank/begin/weighting/plots/brute/pr_tmoy_b_'+SE+'_moy_pondere_deltas_exp_mc_tmoy_2071_2100_'+saison[s]+'_'+rcp,bbox_inches='tight')
else:
    plt.title('Delta entre 1981-2010 et 2071-2100\n'+saison[s]+'_'+rcp+' ('+SE+')\nPost-traitées')
   # plt.savefig('/tank/begin/weighting/plots/posttraite/pr_tmoy_pt_'+SE+'_moy_pondere_deltas_exp_mc_tmoy_2071_2100_'+saison[s]+'_'+rcp,bbox_inches='tight')
