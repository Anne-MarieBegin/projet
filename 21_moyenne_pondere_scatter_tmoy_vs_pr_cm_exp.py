...
######################################################################
if b_pt == 'brute':
    df_diff=pd.read_csv('/tank/begin/weighting/resultats/pond_exp_mc_obs_b_tmoy_'+saison[s]+'_'+rcp+'.csv')
else:
    df_diff=pd.read_csv('/tank/begin/weighting/resultats/pond_exp_mc_obs_pt_tmoy_'+saison[s]+'_'+rcp+'.csv')
...
for j in range(len(dff1_)):
    moyd.append(dff1_['data'][j][s]*w_diff[j])
moy_pond_exp_1=(np.sum(moyd)/np.sum(w_diff))

dff2_=df2.iloc[position].reset_index()
moyd=[]
for j in range(len(dff2_)):
    moyd.append(dff2_['data'][j][s]*w_diff[j])
moy_pond_exp_2=(np.sum(moyd)/np.sum(w_diff))

dff3_=df3.iloc[position].reset_index()
moyd=[]
for j in range(len(dff3_)):
    moyd.append(dff3_['data'][j][s]*w_diff[j])
moy_pond_exp_3=(np.sum(moyd)/np.sum(w_diff))
...
for j in range(len(dff1p_)):
    moyd.append(dff1p_['data'][j][s]*w_diff[j])
moy_pond_exp_1p=(np.sum(moyd)/np.sum(w_diff))

dff2p_=df2p.iloc[position].reset_index()
moyd=[]
for j in range(len(dff2p_)):
    moyd.append(dff2p_['data'][j][s]*w_diff[j])
moy_pond_exp_2p=(np.sum(moyd)/np.sum(w_diff))

dff3p_=df3p.iloc[position].reset_index()
moyd=[]
for j in range(len(dff3p_)):
    moyd.append(dff3p_['data'][j][s]*w_diff[j])
moy_pond_exp_3p=(np.sum(moyd)/np.sum(w_diff))
...
legend=['moyenne non pondérée','moy pondérée_250000km²(5$\Delta$x)',
        ...
        'CMIP5','CORDEX','ClimEX','moy_pondérée_moyenne_climatique_tmoy_obs_'+saison[s],'moy_pond_exp_moyenne_climatique_tmoy_obs_'+saison[s]
        ]
