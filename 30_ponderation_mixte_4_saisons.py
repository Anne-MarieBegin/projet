...
if b_pt == 'brute':
    df_exp_mc_djf=pd.read_csv('/tank/begin/weighting/resultats/pond_exp_pente_obs_b_pr_DJF_'+rcp+'.csv')
    df_exp_mc_mam=pd.read_csv('/tank/begin/weighting/resultats/pond_exp_pente_obs_b_pr_MAM_'+rcp+'.csv')
    df_exp_mc_jja=pd.read_csv('/tank/begin/weighting/resultats/pond_exp_pente_obs_b_pr_JJA_'+rcp+'.csv')
    df_exp_mc_son=pd.read_csv('/tank/begin/weighting/resultats/pond_exp_pente_obs_b_pr_SON_'+rcp+'.csv')

    #df_1m1v=pd.read_csv('/tank/begin/weighting/resultats/df_'+rcp+'_1m1v_uniforme.csv',delimiter=';',header=2)
else:
    df_exp_mc_djf=pd.read_csv('/tank/begin/weighting/resultats/pond_exp_pente_obs_pt_pr_DJF_'+rcp+'.csv')
    df_exp_mc_mam=pd.read_csv('/tank/begin/weighting/resultats/pond_exp_pente_obs_pt_pr_MAM_'+rcp+'.csv')
    df_exp_mc_jja=pd.read_csv('/tank/begin/weighting/resultats/pond_exp_pente_obs_pt_pr_JJA_'+rcp+'.csv')
    df_exp_mc_son=pd.read_csv('/tank/begin/weighting/resultats/pond_exp_pente_obs_pt_pr_SON_'+rcp+'.csv')
...
w_exp_mc_djf=df_exp_mc_djf['0']
w_exp_mc_mam=df_exp_mc_mam['0']
w_exp_mc_jja=df_exp_mc_jja['0']
w_exp_mc_son=df_exp_mc_son['0']
#w_exp_pente=df_exp_pente['0']
#w_1m1v=dff_1m1v['1m1v_uniforme']
w_exp=[]
for i in range(len(w_exp_mc_djf)):
    w_exp.append(w_exp_mc_djf[i]*w_exp_mc_mam[i]*w_exp_mc_jja[i]*w_exp_mc_son[i])
...
#if b_pt=='brute':
#    df_w_all_norm.to_csv('/tank/begin/weighting/pond_combine/pond_mixte_pente_DJF+MAM+JJA+SON_obs_b_'+var+'_'+rcp+'.csv')
#else:
#    df_w_all_norm.to_csv('/tank/begin/weighting/pond_combine/pond_mixte_pente_DJF+MAM+JJA+SON_obs_pt_'+var+'_'+rcp+'.csv')
...
if b_pt == 'brute':
    plt.title('Poids référence NRcan\n'+var+' pente_DJF+MAM+JJA+SON (E_1)_'+rcp+'\nBrutes',fontsize=15)
    #plt.savefig('/tank/begin/weighting/plots/brute/'+var+'_E_1_b_pond_mixte_pente_DJF+MAM+JJA+SON_obs_'+rcp,bbox_inches='tight')
else:
    plt.title('Poids référence NRcan\n'+var+' pente_DJF+MAM+JJA+SON (E_1)_'+rcp+'\nPost-traitées',fontsize=15)
    #plt.savefig('/tank/begin/weighting/plots/posttraite/'+var+'_E_1_pt_pond_mixte_pente_DJF+MAM+JJA+SON_obs_'+rcp,bbox_inches='tight')
