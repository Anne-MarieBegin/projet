...
v20201218 cycle interannuel(couleur courbes marker),cycle interannuel - tendance,rapport variance avec et sans tendance, pente tendance
v20201221 cycle interannuel(bleu et rouge sans marker),cycle interannuel - tendance(bleu et rouge sans marker),rapport variance avec et sans tendance, pente tendance
v20201222 cycle interannuel (retour lignes couleur),rapport variance avec tendance,pente tendance(égaliser échelle)(corriger rapport écarttype pour variance)
...
#calcul correlation coefficient
#cc=[]
#for k in range(0,len(files)+1):
#    cc.append(np.corrcoef(da[0],da[k]))
#    
...
#var_inter_annuel_sans=[]
#for g in range(0,len(files)+1):
#    var_inter_annuel_sans.append(x_detrended[g].var())
    
#rapport_var_sans=[]
#for h in range(0,len(files)+1):
#    rapport_var_sans.append(var_inter_annuel_sans[h]/var_inter_annuel_sans[0])
...
for p in range(0,13):
    for t in range(0,28):
        yy[p].append((pente[p]/10)*x[t]+oo[p])#(/10 pour ramener ligne 147)
...
#plot pente sur 10 ans de la tendance
plt.figure(4)
plt.plot(sim[:],pente[:],'.r',markersize=12)
plt.xticks(rotation=270)
plt.ylabel('Pente tendance (mm/jour / 10 ans)')
plt.ylim(-0.05,.2)
if b_pt == 'brute':
    plt.title('Précipitation \nSérie annuelle 1971-2000*\nBrutes')
