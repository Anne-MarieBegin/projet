...
for i in range(len(df1)):
    for j in range(len(gcm)):
        if (df1['gcm'][i]==gcm[j])&(df1['resolution'][i]=='NotApplicable')&(df1['rcp'][i]==rcp):
            print(i,gcm[j])
            ii[j].append((df2['data'][i]-df1['data'][i]))
moy_delta_1=[]
for i in ii:
...
if b_pt=='brute':
    ...
    plt.savefig('/tank/begin/weighting/plots/brute/'+var+'_b_ECS_VS_delta_'+stat2[-10:-1]+'_'+rcp,bbox_inches='tight')
else:
    plt.title(var+'\n'+stat2[-10:-1]+'_'+rcp+'\nCoefficient corrélation ='+str(round(cc[0][1],3))+'\nPost-traitées')  
    plt.savefig('/tank/begin/weighting/plots/posttraite/'+var+'_pt_ECS_VS_delta_'+stat2[-10:-1]+'_'+rcp,bbox_inches='tight')
...
for i in range(len(df1)):
    for j in range(len(gcm)):
        if (df1['gcm'][i]==gcm[j])&(df1['resolution'][i]=='NotApplicable')&(df1['rcp'][i]==rcp):
            print(i,gcm[j])
            ii[j].append((df3['data'][i]-df1['data'][i]))
moy_delta_2=[]
for i in ii:
...
if b_pt=='brute':
    plt.title(var+'\n'+stat3[-10:-1]+'_'+rcp+'\nCoefficient corrélation ='+str(round(cc[0][1],3))+'\nBrutes')
    plt.savefig('/tank/begin/weighting/plots/brute/'+var+'_b_ECS_VS_delta_'+stat3[-10:-1]+'_'+rcp,bbox_inches='tight')
else:
    plt.title(var+'\n'+stat3[-10:-1]+'_'+rcp+'\nCoefficient corrélation ='+str(round(cc[0][1],3))+'\nPost-traitées')  
    plt.savefig('/tank/begin/weighting/plots/posttraite/'+var+'_pt_ECS_VS_delta_'+stat3[-10:-1]+'_'+rcp,bbox_inches='tight')
